...
public class LevelFlowField {
	...
	public void Build(SquareCoord destination) {
		...
		Queue<SquareCoord> q = new Queue<SquareCoord>();
		distance[destination] = 0;
		q.Enqueue(destination);

		// BFS to compute distances
		while (q.Count > 0) {
			SquareCoord cur = q.Dequeue();
			int curDist = distance[cur];

			foreach (SquareCoord nb in GetNeighbors(cur)) {
				// Skip if already visited
				if (distance.ContainsKey(nb))
					continue;

				// Skip if cell doesn't exist or is not walkable
				if (!grid.TryGetCell(nb, out LevelCell nbCell) || !IsCellWalkable(nbCell))
					continue;
		...
		foreach (KeyValuePair<SquareCoord, int> kv in distance) {
			SquareCoord coord = kv.Key;
			...
			foreach (SquareCoord nb in GetNeighbors(coord)) {
	...
	public List<SquareCoord> GetPath(SquareCoord source, int maxSteps = 10000) {
		List<SquareCoord> path = new List<SquareCoord>();

		if (!distance.ContainsKey(source))
			return path; // unreachable

		SquareCoord cur = source;
		for (int i = 0; i < maxSteps; i++) {
			path.Add(cur);

			if (!flow.TryGetValue(cur, out SquareCoord next))
				break;
	...
	private IEnumerable<SquareCoord> GetNeighbors(SquareCoord coord) {
		foreach (SquareCoord v in SquareCoord.DirectionVectors) {
